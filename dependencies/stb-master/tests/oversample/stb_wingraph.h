...
STB_EXTERN int stbwingraph_MessageBox(
    stbwingraph_hwnd win, unsigned int type, char *caption, char *text, ...);
STB_EXTERN int stbwingraph_ChangeResolution(
    unsigned int w, unsigned int h, unsigned int bits, int use_message_box);
STB_EXTERN int stbwingraph_SetPixelFormat(
    stbwingraph_hwnd win,
    int color_bits,
    int alpha_bits,
    int depth_bits,
    int stencil_bits,
...
STB_EXTERN void *stbwingraph_CreateWindow(
    int primary,
    ...
    int dest_alpha,
    int stencil);
STB_EXTERN void *stbwingraph_CreateWindowSimple(
    stbwingraph_window_proc func, int width, int height);
STB_EXTERN void *stbwingraph_CreateWindowSimpleFull(
    stbwingraph_window_proc func, int fullscreen, int ww, int wh, int fw, int fh);
    ...
    unsigned int w, unsigned int h, unsigned int bits, int use_message_box)
{
    ...
    {
        if (use_message_box)
            stbwingraph_MessageBox(
                stbwingraph_primary_window,
                MB_ICONERROR,
                NULL,
                "The resolution %d x %d x %d-bits is not supported.",
                w,
                h,
                bits);
        return FALSE;
        ...
        default:
            if (use_message_box)
                stbwingraph_MessageBox(
                    ...
                    MB_ICONERROR,
                    NULL,
                    "An unknown error prevented a change to a %d x %d x %d-bit display.",
                    w,
                    h,
                    bits);
            return FALSE;
...
int stbwingraph_SetPixelFormat(
    stbwingraph_hwnd win,
    int color_bits,
    int alpha_bits,
    int depth_bits,
...
static void stbwingraph__mouse(
    stbwingraph_event *e,
    int type,
    WPARAM wparam,
    LPARAM lparam,
...
static void stbwingraph__mousewheel(
    stbwingraph_event *e,
    int type,
    WPARAM wparam,
    LPARAM lparam,
...
static int WINAPI stbwingraph_WinProc(HWND wnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    switch (msg)
    {
        case WM_CREATE:
            {
                ...
                z->dc = GetDC(wnd);
                if (stbwingraph_SetPixelFormat(
                        wnd, z->color, z->alpha, z->depth, z->stencil, z->accum))
                {
                    ...
                    if (z->rc)
                    {
                        ...
                        if (z->share_window)
                        {
                            stbwingraph__window *y =
                                (stbwingraph__window *)GetWindowLong(z->share_window, GWL_USERDATA);
        ...
        case WM_CHAR:
            stbwingraph__key(&e, STBWGE_char, wparam, z);
            break;
        case WM_KEYDOWN:
            stbwingraph__key(&e, STBWGE_keydown, wparam, z);
            break;
        case WM_KEYUP:
            stbwingraph__key(&e, STBWGE_keyup, wparam, z);
            break;

        case WM_NCMOUSEMOVE:
            stbwingraph__inclient(z, 0);
            break;
        case WM_MOUSEMOVE:
            stbwingraph__inclient(z, 1);
            stbwingraph__mouse(&e, STBWGE_mousemove, wparam, lparam, 0, wnd, z);
            break;
        case WM_LBUTTONDOWN:
            stbwingraph__mouse(&e, STBWGE_leftdown, wparam, lparam, 1, wnd, z);
            break;
        case WM_MBUTTONDOWN:
            stbwingraph__mouse(&e, STBWGE_middledown, wparam, lparam, 1, wnd, z);
            break;
        case WM_RBUTTONDOWN:
            stbwingraph__mouse(&e, STBWGE_rightdown, wparam, lparam, 1, wnd, z);
            break;
        case WM_LBUTTONUP:
            stbwingraph__mouse(&e, STBWGE_leftup, wparam, lparam, -1, wnd, z);
            break;
        case WM_MBUTTONUP:
            stbwingraph__mouse(&e, STBWGE_middleup, wparam, lparam, -1, wnd, z);
            break;
        case WM_RBUTTONUP:
            stbwingraph__mouse(&e, STBWGE_rightup, wparam, lparam, -1, wnd, z);
            break;
        case WM_MOUSEWHEEL:
            stbwingraph__mousewheel(&e, STBWGE_mousewheel, wparam, lparam, 0, wnd, z);
            break;
...
void *stbwingraph_CreateWindowSimpleFull(
    stbwingraph_window_proc func, int fullscreen, int ww, int wh, int fw, int fh)
{
    if (fullscreen == -1)
    {
...
int stbwingraph_MainLoop(stbwingraph_update func, float mintime)
{
    ...
    for (;;)
    {
        ...
        if (needs_drawing || is_animating)
        {
            ...
            {
                stbwingraph__window *z =
                    (stbwingraph__window *)GetWindowLong(stbwingraph_primary_window, GWL_USERDATA);
                if (z && !z->active)
                {
                    real = 0;
                }
...
void stbwingraph_SwapBuffers(void *win)
{
    stbwingraph__window *z;
    if (win == NULL)
        win = stbwingraph_primary_window;
