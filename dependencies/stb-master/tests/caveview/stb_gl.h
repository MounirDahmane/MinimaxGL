...
extern "C"
{
    ...
    extern void stbgl_Perspective(
        float zoom, float max_hfov, float max_vfov, float znear, float zfar);
    extern void stbgl_PerspectiveViewport(
        int x,
        int y,
        int w,
        int h,
    ...
    extern void stbgl_drawRectTC(
        float x0, float y0, float x1, float y1, float s0, float t0, float s1, float t1);
    ...
    extern int stbgl_TestTextureEx(
        int w, char *scale_table, int checks_log2, int r1, int g1, int b1, int r2, int b2, int g2);
    extern unsigned int stbgl_rand(
        void); // internal, but exposed just in case; LCG, so use middle bits

    extern int stbgl_TexImage2D(int texid, int w, int h, void *data, char *props);
    extern int stbgl_TexImage2D_Extra(
        int texid, int w, int h, void *data, int chan, char *props, int preserve_data);
...
void stbgl_PerspectiveViewport(
    int x, int y, int w, int h, float zoom, float min_hfov, float min_vfov, float znear, float zfar)
{
    if (znear <= 0.0001f)
        znear = 0.0001f;
    glViewport(x, y, w, h);
...
void stbgl_initCamera_zup_facing_y(void) { glRotatef(-90, 1, 0, 0); }
...
{
    char scale_table[] = {10, 20, 30, 30, 35, 40, 5, 18, 25, 13, 7, 5, 3, 3, 2, 2, 2,
...
static void(__stdcall *stbgl__CompressedTexImage2DARB)(
    int target,
    int level,
    int internalformat,
    int width,
...
#else
static void (*stbgl__CompressedTexImage2DARB)(
    int target,
    int level,
    int internalformat,
    int width,
...
static void stbgl__convert(uint8 *p, uint8 *q, int n, int input_desc, uint8 *end)
{
    ...
    switch (input_desc)
    {
        ...
        case GL_LUMINANCE:
            for (i = 0; i < n; ++i, p += 4)
                p[0] = p[1] = p[2] = q[0], p[3] = 255, q += 1;
            break;
        case GL_ALPHA:
            for (i = 0; i < n; ++i, p += 4)
                p[0] = p[1] = p[2] = 0, p[3] = q[0], q += 1;
            break;
        case GL_LUMINANCE_ALPHA:
            for (i = 0; i < n; ++i, p += 4)
                p[0] = p[1] = p[2] = q[0], p[3] = q[1], q += 2;
            break;
        case GL_RGB:
            for (i = 0; i < n; ++i, p += 4)
                p[0] = q[0], p[1] = q[1], p[2] = q[2], p[3] = 255, q += 3;
            break;
        case GL_RGBA:
            memcpy(p, q, n * 4);
            break;
    ...
    int texid, int w, int h, void *data, int chan, char *props, int preserve_data)
{
    ...
    while (*props)
    {
        ...
        {
            case '1':
                input_desc = GL_LUMINANCE;
                break;
            case '2':
                input_desc = GL_LUMINANCE_ALPHA;
                break;
            case '3':
                input_desc = GL_RGB;
                break;
            ...
            case 'a':
                ...
                break;
            case 'r':
                if (stbgl_m(props, "rgba"))
                {
                    input_desc = GL_RGBA;
                    props += 3;
                    break;
                }
                if (stbgl_m(props, "rgb"))
                {
                    input_desc = GL_RGB;
                    props += 2;
                    break;
                ...
                break;
            case 'y':
                if (stbgl_m(props, "ycocg"))
                {
                    if (props[5] == 'j')
                    {
                        props += 5;
                        input_desc = STBGL_YCOCGJ;
                    }
                    else
                    {
                        props += 4;
                        input_desc = STBGL_YCOCG;
                    }
                    break;
                }
                return 0;
            case 'L':
                if (props[1] == 'A')
                {
                    output_desc = GL_LUMINANCE_ALPHA;
                    ++props;
                }
                else
                    output_desc = GL_LUMINANCE;
                break;
            case 'I':
                output_desc = GL_INTENSITY;
                break;
            case 'A':
                output_desc = GL_ALPHA;
                break;
            case 'R':
                if (stbgl_m(props, "RGBA"))
                {
                ...
                break;
            case 'Y':
                if (stbgl_m(props, "YCoCg") || stbgl_m(props, "YCOCG"))
                {
                    props += 4;
                ...
                return 0;
            case 'D':
                if (stbgl_m(props, "DXT"))
                {
                    switch (props[3])
                    {
                        case '1':
                            output_desc = STBGL_COMPRESSED_RGB_S3TC_DXT1;
                            break;
                        case '3':
                            output_desc = STBGL_COMPRESSED_RGBA_S3TC_DXT3;
                            break;
                        case '5':
            ...
            case 'C':
                if (wrap_s == STBGL_UNDEFINED)
                    wrap_s = STBGL_CLAMP_TO_BORDER;
                else
                    wrap_t = STBGL_CLAMP_TO_BORDER;
                break;
            case 'c':
                if (wrap_s == STBGL_UNDEFINED)
                    wrap_s = STBGL_CLAMP_TO_EDGE;
                else
                    wrap_t = STBGL_CLAMP_TO_EDGE;
                break;
            case 'f':
                input_type = GL_FLOAT;
                break;
...
#ifdef STB_COMPRESS_DXT_BLOCK
    if (!is_compressed || !stbgl__CompressedTexImage2DARB ||
        output_desc == STBGL_COMPRESSED_RGBA_S3TC_DXT3 || data == NULL)
#endif
    {
        switch (mipmaps)
        {
            ...
            case STBGL_UNDEFINED:
                ...
                if (non_power_of_two || ((w & (w - 1)) == 0 && (h & (h - 1)) == 0))
                    glTexImage2D(
                        GL_TEXTURE_2D, 0, output_desc, w, h, 0, input_desc, input_type, data);
                else
                    gluBuild2DMipmaps(
                        GL_TEXTURE_2D, output_desc, w, h, input_desc, input_type, data);
            ...
            case STBGL_MIPMAPS:
                {
                    ...
                    if (data == NULL)
                        size = 0; // reuse same block of memory for all mipmaps
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            stbgl__CompressedTexImage2DARB(
                ...
                h,
                0,
                ((w + 3) & ~3) * ((h + 3) & ~3) / 16 * osize,
                out);
            // glTexImage2D(GL_TEXTURE_2D, level, alpha?GL_RGBA:GL_RGB, w, h, 0, GL_RGBA,
            // GL_UNSIGNED_BYTE, rgba ? rgba : data);

            if (mipmaps == STBGL_UNDEFINED)
                break;
            if (w <= 1 && h <= 1)
                break;
            if (mipmaps == STBGL_MIPMAPS)
                data = (void *)((char *)data + w * h * size * chan);
