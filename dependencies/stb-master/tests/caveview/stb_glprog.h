...
extern "C"
{
    ...
    extern GLuint stbgl_create_program(
    ...
    //             char **vertex_source,   // NULL or one or more strings with the vertex shader
    //             source, with a final NULL char **frag_source,     // NULL or one or more strings
    //             with the fragment shader source, with a final NULL char **binds,           //
    //             NULL or zero or more strings with attribute bind names, with a final NULL char
    //             *error,            // output location where compile error message is placed int
    //             error_buflen)       // length of error output buffer
    //
    ...
    extern GLuint stbgl_compile_shader(
        GLenum type, char const **sources, int num_sources, char *error, int error_buflen);
    ...
    extern GLuint stbgl_link_program(
    ...
    //
    //    vertex_shader    a compiled vertex shader from stbgl_compile_shader, or 0 for
    //    fixed-function (if legal) fragment_shader  a compiled fragment shader from
    //    stbgl_compile_shader, or 0 for fixed-function (if legal)
    //
    ...
    extern void stbgl_find_uniforms(
        GLuint prog, GLint *locations, char const **uniforms, int num_uniforms);
    ...
    extern void stbglVertexAttribPointer(
        GLuint index,
        GLint size,
        GLenum type,
...
STB_GLPROG_DECLARE GLuint stbgl_compile_shader(
    GLenum type, char const **sources, int num_sources, char *error, int error_buflen)
{
    ...
    if (!shader)
    {
        stb_glprog_error(
            error,
            error_buflen,
            "Couldn't allocate shader object in stbgl_compile_shader for ",
            typename,
            NULL);
        return 0;
    }
...
STB_GLPROG_DECLARE GLuint stbgl_link_program(
    GLuint vertex_shader,
    GLuint fragment_shader,
    char const **binds,
    int num_binds,
    ...
    int error_buflen)
{
    ...
    if (!prog)
    {
        stb_glprog_error(
            error,
            error_buflen,
            "Couldn't allocate program object in stbgl_link_program",
            NULL,
            NULL);
        return 0;
    }
...
STB_GLPROG_DECLARE GLuint stbgl_create_program(
    char const **vertex_source,
    char const **frag_source,
    char const **binds,
    char *error,
    int error_buflen)
{
    ...
    {
        fragment =
            stbgl_compile_shader(STBGL_FRAGMENT_SHADER, frag_source, -1, error, error_buflen);
        if (fragment)
...
STB_GLPROG_DECLARE void stbgl_delete_shader(GLuint shader) { stbglDeleteShader(shader); }

STB_GLPROG_DECLARE void stgbl_delete_program(GLuint program) { stbglDeleteProgram(program); }
...
STB_GLPROG_DECLARE void stbgl_find_uniforms(
    GLuint prog, GLint *locations, char const **uniforms, int num_uniforms)
{
    int i;
    if (num_uniforms < 0)
...
STB_GLPROG_DECLARE void stbglUseProgram(GLuint program) { stbgl_UseProgram(program); }
...
STB_GLPROG_DECLARE void stbglVertexAttribPointer(
    GLuint index,
    GLint size,
    GLenum type,
    GLboolean normalized,
...
STB_GLPROG_DECLARE void stbglEnableVertexAttribArray(GLuint index)
{
    STBGL_ARBIFY(glEnableVertexAttribArray)(index);
}
STB_GLPROG_DECLARE void stbglDisableVertexAttribArray(GLuint index)
{
    STBGL_ARBIFY(glDisableVertexAttribArray)(index);
}

STB_GLPROG_DECLARE void stbglUniform1fv(GLint loc, GLsizei count, const GLfloat *v)
{
    STBGL_ARBIFY(glUniform1fv)(loc, count, v);
}
STB_GLPROG_DECLARE void stbglUniform2fv(GLint loc, GLsizei count, const GLfloat *v)
{
    STBGL_ARBIFY(glUniform2fv)(loc, count, v);
}
STB_GLPROG_DECLARE void stbglUniform3fv(GLint loc, GLsizei count, const GLfloat *v)
{
    STBGL_ARBIFY(glUniform3fv)(loc, count, v);
}
STB_GLPROG_DECLARE void stbglUniform4fv(GLint loc, GLsizei count, const GLfloat *v)
{
    STBGL_ARBIFY(glUniform4fv)(loc, count, v);
}

STB_GLPROG_DECLARE void stbglUniform1iv(GLint loc, GLsizei count, const GLint *v)
{
    STBGL_ARBIFY(glUniform1iv)(loc, count, v);
}
STB_GLPROG_DECLARE void stbglUniform2iv(GLint loc, GLsizei count, const GLint *v)
{
    STBGL_ARBIFY(glUniform2iv)(loc, count, v);
}
STB_GLPROG_DECLARE void stbglUniform3iv(GLint loc, GLsizei count, const GLint *v)
{
    STBGL_ARBIFY(glUniform3iv)(loc, count, v);
}
STB_GLPROG_DECLARE void stbglUniform4iv(GLint loc, GLsizei count, const GLint *v)
{
    STBGL_ARBIFY(glUniform4iv)(loc, count, v);
}

STB_GLPROG_DECLARE void stbglUniform1f(GLint loc, float v0) { STBGL_ARBIFY(glUniform1f)(loc, v0); }
...
STB_GLPROG_DECLARE void stbglUniform1i(GLint loc, GLint v0) { STBGL_ARBIFY(glUniform1i)(loc, v0); }
