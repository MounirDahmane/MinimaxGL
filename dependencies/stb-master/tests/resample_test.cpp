...
// Avoiding use of the system rand() to be sure that our tests generate the same test data on any
// system.
...
void performance(int argc, char **argv)
{
    ...
    out_w = w * 2;
    out_h = h / 4;
    count = 20; // 2   // note this is structured pessimily, would be much faster to downsample
                // vertically first
#elif 0
    out_w = w / 4;
    out_h = h * 2;
    ...
    for (i = 0; i < count; ++i)
        if (srgb)
            stbir_resize_uint8_srgb(
                input_pixels, w, h, 0, output_pixels, out_w, out_h, 0, n, -1, 0);
        else
            stbir_resize(
                input_pixels,
                w,
...
template <typename F, typename T> void convert_image(const F *input, T *output, int length)
{
...
void test_channels(const char *file, float width_percent, float height_percent, int channels)
{
    ...
    unsigned char *channels_data =
        (unsigned char *)malloc(w * h * channels * sizeof(unsigned char));
    ...
    unsigned char *output_data =
        (unsigned char *)malloc(new_w * new_h * channels * sizeof(unsigned char));
    ...
    float t1)
{
    ...
    sprintf(
        output,
        "test-output/subpixel-region-%d-%d-%f-%f-%f-%f-%s",
        new_w,
        new_h,
        s0,
        t0,
    ...
    float y_offset)
{
    ...
    sprintf(
        output,
        "test-output/subpixel-command-%d-%d-%f-%f-%f-%f-%s",
        new_w,
        new_h,
        x_scale,
        y_scale,
...
void verify_box(void)
{
    ...
    for (j = 0; j < 4; ++j)
        for (i = 0; i < 4; ++i)
        {
            int n = image88[j * 2 + 0][i * 2 + 0] + image88[j * 2 + 0][i * 2 + 1] +
                    image88[j * 2 + 1][i * 2 + 0] + image88[j * 2 + 1][i * 2 + 1];
            STBIR_ASSERT(
                output44[j][i] == ((n + 2) >> 2) ||
                output44[j][i] ==
                    ((n + 1) >> 2)); // can't guarantee exact rounding due to numerical precision
            t += n;
        }
    STBIR_ASSERT(
        output11[0][0] == ((t + 32) >> 6) ||
        output11[0][0] ==
            ((t + 31) >> 6)); // can't guarantee exact rounding due to numerical precision
}
...
void test_filters(void)
{
    ...
    // checkerboard
    {
        ...
        stbir_resize_uint8_srgb_edgemode(
            input[0], 64, 64, 0, output[0], 16, 16, 0, 1, -1, 0, STBIR_EDGE_WRAP);
        for (j = 0; j < 16; ++j)
            for (i = 0; i < 16; ++i)
