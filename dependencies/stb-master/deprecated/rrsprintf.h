...
#define RR_SPRINTF_DECORATE(name)                                                                  \
    rr##name // define this before including if you want to change the names
#endif
...
RRPUBLIC_DEF int RR_SPRINTF_DECORATE(vsprintfcb)(
    RRSPRINTFCB *callback, void *user, char *buf, char const *fmt, va_list va);
...
#ifndef rUINTa
#if defined(__ppc64__) || defined(__aarch64__) || defined(_M_X64) || defined(__x86_64__) ||        \
    defined(__x86_64)
#define rUINTa rU64
#else
#define rUINTa rU32
...
#ifdef RR_SPRINTF_NOUNALIGNED // define this before inclusion to force rrsprint to always use
                              // aligned accesses
...
static rS32 rrreal_to_str(
    char const **start, rU32 *len, char *out, rS32 *decimal_pos, double value, rU32 frac_digits);
...
static char RRcomma = ',';
static char rrdiglookup[201] = "0001020304050607080910111213141516171819202122232425262728293031323"
...
#define flush_cb()                                                                                 \
    {                                                                                              \
        chk_cb_bufL(RR_SPRINTF_MIN - 1);                                                           \
    } // flush if there is even one byte in the buffer
#define cb_buf_clamp(cl, v)                                                                        \
    cl = v;                                                                                        \
    if (callback)                                                                                  \
    {                                                                                              \
        int lg = RR_SPRINTF_MIN - (int)(bf - buf);                                                 \
        if (cl > lg)                                                                               \
            cl = lg;                                                                               \
    }
        ...
        for (;;)
        {
            ...
            for (;;)
            {
                ...
                if ((v - 0x01010101) & c)
                    goto schk2;
                if (callback)
                    if ((RR_SPRINTF_MIN - (int)(bf - buf)) < 4)
                        goto schk1;
                *(rU32 *)bf = v;
                bf += 4;
                f += 4;
            ...
            case 'd': // integer
                ...
                for (;;)
                {
                    ...
                    if ((fl & CS) == 0)
                    {
                        ...
                        {
                            s -= 2;
                            *(rU16 *)s = *(rU16 *)&rrdiglookup[(n % 100) * 2];
                            n /= 100;
                        }
                    }
                    while (n)
...
static double const rrbot[23] = {1e+000, 1e+001, 1e+002, 1e+003, 1e+004, 1e+005, 1e+006, 1e+007,
                                 1e+008, 1e+009, 1e+010, 1e+011, 1e+012, 1e+013, 1e+014, 1e+015,
                                 1e+016, 1e+017, 1e+018, 1e+019, 1e+020, 1e+021, 1e+022};
static double const rrnegbot[22] = {1e-001, 1e-002, 1e-003, 1e-004, 1e-005, 1e-006, 1e-007, 1e-008,
                                    1e-009, 1e-010, 1e-011, 1e-012, 1e-013, 1e-014, 1e-015, 1e-016,
                                    1e-017, 1e-018, 1e-019, 1e-020, 1e-021, 1e-022};
static double const rrnegboterr[22] = {
...
static double const rrtop[13] = {
    ...
    1e+276,
    1e+299};
static double const rrnegtop[13] = {
    1e-023,
    1e-046,
    1e-069,
    1e-092,
    ...
    1e-276,
    1e-299};
static double const rrtoperr[13] = {
    8388608,
...
static double const rrnegtoperr[13] = {
...
#define rrddmultlo(oh, ol, xh, xl, yh, yl) ol = ol + (xh * yl + xl * yh);

#define rrddmultlos(oh, ol, xh, yl) ol = ol + (xh * yl);

static void rrraise_to_power10(
    double *ohi, double *olo, double d, rS32 power) // power can be -323 to +350
{
    double ph, pl;
    if ((power >= 0) && (power <= 22))
    {
    ...
    else
    {
        ...
        if (power < 0)
        {
            ...
            {
                --eb;
                rrddmulthi(ph, pl, d, rrnegbot[eb]);
                rrddmultlos(ph, pl, d, rrnegboterr[eb]);
            }
            if (et)
            {
                rrddrenorm(ph, pl);
                --et;
                rrddmulthi(p2h, p2l, ph, rrnegtop[et]);
                rrddmultlo(p2h, p2l, ph, pl, rrnegtop[et], rrnegtoperr[et]);
                ph = p2h;
                pl = p2l;
            }
        }
        else
        {
            if (eb)
            {
                ...
                {
                    rrddrenorm(ph, pl);
                    rrddmulthi(p2h, p2l, ph, rrbot[e]);
                    rrddmultlos(p2h, p2l, rrbot[e], pl);
                    ph = p2h;
                    pl = p2l;
                }
            }
            if (et)
            {
                rrddrenorm(ph, pl);
                --et;
                rrddmulthi(p2h, p2l, ph, rrtop[et]);
                rrddmultlo(p2h, p2l, ph, pl, rrtop[et], rrtoperr[et]);
                ph = p2h;
                pl = p2l;
            }
        }
    ...
    char const **start, rU32 *len, char *out, rS32 *decimal_pos, double value, rU32 frac_digits)
{
    ...
    {
        rU32 dg = 1;
        if ((rU64)bits >= rrpot[9])
            dg = 10;
        while ((rU64)bits >= rrpot[dg])
        {
            ++dg;
            if (dg == 20)
                goto noround;
    ...
    for (;;)
    {
        ...
        {
            out -= 2;
            *(rU16 *)out = *(rU16 *)&rrdiglookup[(n % 100) * 2];
            n /= 100;
            e += 2;
        }
        if (bits == 0)
