...
void ImGui::TextAlignedV(float align_x, float size_x, const char *fmt, va_list args)
{
    ...
    if (size_x < text_size.x &&
        IsItemHovered(
            ImGuiHoveredFlags_NoNavOverride | ImGuiHoveredFlags_AllowWhenDisabled |
...
bool ImGui::InvisibleButton(const char *str_id, const ImVec2 &size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(
        ImVec2(
            size_arg.x != 0.0f ? size_arg.x : -FLT_MIN, size_arg.y != 0.0f ? size_arg.y : -FLT_MIN),
        0.0f,
        0.0f);
...
void ImGui::ExtendHitBoxWhenNearViewportEdge(
...
bool ImGui::Checkbox(const char *label, bool *v)
{
    ...
    if (is_visible)
    {
        ...
        if (mixed_value)
        {
            ...
            ImVec2 pad(
                ImMax(1.0f, IM_TRUNC(square_sz / 3.6f)), ImMax(1.0f, IM_TRUNC(square_sz / 3.6f)));
            window->DrawList->AddRectFilled(
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        const ImRect bb(
            ImVec2(window->DC.CursorPos.x, y1), ImVec2(window->DC.CursorPos.x + thickness, y2));
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        /// Should change.
        const ImRect bb(
            ImVec2(x1, window->DC.CursorPos.y), ImVec2(x2, window->DC.CursorPos.y + thickness));
...
void ImGui::SeparatorTextEx(ImGuiID id, const char *label, const char *label_end, float extra_w)
{
    ...
    if (label_size.x > 0.0f)
    {
        ...
        if (sep1_x2 > sep1_x1 && separator_thickness > 0.0f)
            window->DrawList->AddLine(
                ImVec2(sep1_x1, seps_y),
                ImVec2(sep1_x2, seps_y),
                separator_col,
                separator_thickness);
        if (sep2_x2 > sep2_x1 && separator_thickness > 0.0f)
            window->DrawList->AddLine(
                ImVec2(sep2_x1, seps_y),
...
bool ImGui::BeginCombo(const char *label, const char *preview_value, ImGuiComboFlags flags)
{
    ...
    if (label_size.x > 0)
        RenderText(
            ImVec2(bb.Max.x + style.ItemInnerSpacing.x, bb.Min.y + style.FramePadding.y), label);
...
static bool TempInputIsClampEnabled(
    ImGuiSliderFlags flags, ImGuiDataType data_type, const void *p_min, const void *p_max)
{
    ...
    ImRect *out_grab_bb)
{
    ...
    if (!is_floating_point && v_range_f >= 0.0f) // v_range_f < 0 may happen on integer overflows
        grab_sz = ImMax(
            slider_sz / (v_range_f + 1), style.GrabMinSize); // For integer sliders: if possible
    ...
    ImGuiInputTextFlags flags)
{
    ...
    IM_ASSERT(
        ...
        0); // Not supported by InputScalar(). Please open an issue if you this would be useful to
            // you. Otherwise use IsItemDeactivatedAfterEdit()!
...
// word-wrapping.
static int STB_TEXTEDIT_MOVELINESTART_IMPL(
    ImGuiInputTextState *obj, ImStb::STB_TexteditState *state, int cursor)
{
...
static int STB_TEXTEDIT_MOVELINEEND_IMPL(
    ImGuiInputTextState *obj, ImStb::STB_TexteditState *state, int cursor)
{
...
static int STB_TEXTEDIT_INSERTCHARS(
    ImGuiInputTextState *obj, int pos, const char *new_text, int new_text_len)
{
...
void ImGuiInputTextState::OnKeyPressed(int key)
{
    ...
    if (key_u == STB_TEXTEDIT_K_LEFT || key_u == STB_TEXTEDIT_K_LINESTART ||
        ...
        key_u == STB_TEXTEDIT_K_RIGHT || key_u == STB_TEXTEDIT_K_LINEEND ||
...
ImGuiInputTextCallbackData::ImGuiInputTextCallbackData() { memset((void *)this, 0, sizeof(*this)); }
    ...
    void *callback_user_data)
{
    ...
    if (is_wordwrap)
        wrap_width = ImMax(
            1.0f,
            GetContentRegionAvail().x + (draw_window->ScrollbarY ? 0.0f : -g.Style.ScrollbarSize));
    ...
    const bool init_make_active =
        (input_requested_by_user || input_requested_by_nav || input_requested_by_reactivate ||
         user_scroll_finish);
    ...
    else if ((init_make_active && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (!user_scroll_finish)
        {
            state->TextToRevertTo.resize(
                buf_len + 1); // UTF-8. we use +1 to make sure that .Data is always pointing to at
        ...
        else if (is_paste)
        {
            if (const char *clipboard = GetClipboardText())
            {
                ...
                for (const char *s = clipboard; *s != 0;)
                {
                    ...
                    memcpy(
                        clipboard_filtered.Data + clipboard_filtered.Size - out_len,
                        c_utf8,
                        out_len);
                }
            ...
            if (event_flag)
            {
                ...
                if (callback_data.BufDirty)
                {
                    ...
                    IM_ASSERT(
                        ...
                        (int)ImStrlen(callback_data.Buf)); // You need to maintain BufTextLen if you
                                                           // change the text!
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            const float bg_eol_width = IM_TRUNC(
...
bool ImGui::ColorButton(
    const char *desc_id, const ImVec4 &col, ImGuiColorEditFlags flags, const ImVec2 &size_arg)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
void ImGui::TreeNodeDrawLineToChildNode(const ImVec2 &target_pos)
{
    ...
    if (rounding > 0.0f)
    {
        ...
        window->DrawList->PathStroke(
            GetColorU32(ImGuiCol_TreeLines), ImDrawFlags_None, g.Style.TreeLinesSize);
    }
    else
    {
        window->DrawList->AddLine(
    ...
    if (is_visible)
    {
        ...
        if (highlighted || selected)
        {
            ...
            ImU32 col = GetColorU32(
                (held && highlighted) ? ImGuiCol_HeaderActive
...
static void BoxSelectScrollWithMouseDrag(
    ImGuiBoxSelectState *bs, ImGuiWindow *window, const ImRect &inner_r)
{
    ImGuiContext &g = *GImGui;
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData *ms, ImGuiWindow *window)
{
    ...
    else
    {
        ...
        scope_rect.Min = ImMin(
            scope_rect.Min + ImVec2(window->DecoInnerSizeX1, window->DecoInnerSizeY1),
            scope_rect.Max);
        return scope_rect;
    }
}
...
ImGuiMultiSelectIO *ImGui::BeginMultiSelect(
...
ImGuiMultiSelectIO *ImGui::EndMultiSelect()
{
    ...
    IM_ASSERT_USER_ERROR(
        ms->FocusScopeId == g.CurrentFocusScopeId, "EndMultiSelect() FocusScope mismatch!");
                ...
                g.IO.MouseClickedCount[0] == 1)
            {
                ...
                if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect)
                    SetNavID(
                        ...
                        ImRect(
                            g.IO.MousePos,
                            g.IO.MousePos)); // Automatically switch FocusScope for initial click
...
static void ImGuiSelectionBasicStorage_BatchFinish(
    ImGuiSelectionBasicStorage *selection, bool selected, int size_before_amends)
{
    ImGuiStorage *storage = &selection->_Storage;
...
void ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO *ms_io)
{
    ...
    IM_ASSERT(
        ms_io->ItemsCount != -1 && "Missing value for items_count in BeginMultiSelect() call!");
    ...
    int height_in_items)
{
    ...
    ImVec2 size(
        0.0f,
        ImTrunc(
            GetTextLineHeightWithSpacing() * height_in_items_f + g.Style.FramePadding.y * 2.0f));
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled)
{
    ...
    if (menu_is_open)
    {
        ...
        SetNextWindowPos(
            popup_pos, ImGuiCond_Always); // Note: misleading: the value will serve as reference for
    ...
    const char *label, const char *icon, const char *shortcut, bool selected, bool enabled)
{
    ...
    // another.
    const ImGuiSelectableFlags selectable_flags =
        ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_SelectOnRelease |
    ...
    else
    {
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            ...
            if (shortcut_w > 0.0f)
            {
                ...
                LogSetNextTextDecoration("(", ")");
                RenderText(
                    text_pos + ImVec2(offsets->OffsetShortcut + stretch_w, 0.0f),
                    shortcut,
                    NULL,
                    false);
                PopStyleColor();
...
void ImGui::TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow *window)
{
    ...
    IM_ASSERT(
        g.CurrentTabBar !=
        tab_bar); // Can't work while the tab bar is active as our tab doesn't have an X offset yet,
                  // in theory we could/should test something like (tab_bar->CurrFrameVisible <
                  // g.FrameCount) but we'd need to solve why triggers the commented early-out
                  // assert in BeginTabBarEx() (probably dock node going from implicit to explicit
                  // in same frame)
...
{
    IM_ASSERT(
        (tab_bar->Flags & ImGuiTabBarFlags_DockNode) ==
...
ImVec2 ImGui::TabItemCalcSize(ImGuiWindow *window)
{
    return TabItemCalcSize(
        window->Name, window->HasCloseButton || (window->Flags & ImGuiWindowFlags_UnsavedDocument));
}
    ...
    bool *out_text_clipped)
{
    ...
    const ImVec2 button_pos(
        ImMax(bb.Min.x, bb.Max.x - frame_padding.x - button_sz), bb.Min.y + frame_padding.y);
